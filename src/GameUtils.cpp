...
bool isPositionOnSnake(Vector2 position,
                       const std::vector<Vector2> &snakePosition) {
...
std::string GameUtils::getFormattedGameTime(float startTime, float until) {
  ...
  timeStream << (minutes < 10 ? "0" : "") << minutes << ":"
...
Vector2 GameUtils::getRandomGridPosition() {
  int x =
      GetRandomValue(0, Constants::CELL_AMOUNT_X - 1) * Constants::CELL_SIZE;
  int y =
      GetRandomValue(0, Constants::CELL_AMOUNT_Y - 1) * Constants::CELL_SIZE;
  return {static_cast<float>(x), static_cast<float>(y)};
}

Vector2
GameUtils::getRandomFoodPosition(const std::vector<Vector2> &snakePosition,
  ...
  do {
    ...
    onWall = std::any_of(wallPositions.begin(), wallPositions.end(),
                         [&](const Vector2 &wall) {
                           return position.x == wall.x && position.y == wall.y;
                         });
...
Vector2
GameUtils::getRandomWallPosition(const std::vector<Vector2> &snakePosition,
